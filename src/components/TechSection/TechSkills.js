...
import { technicalSkillsRows } from "./TechData";

// Rename the default function to match the file name
  ...
  return (
    ...
    >
      ...
      <div className={styles.header}>
        ...
        <h2 className={styles.title}>TECHNICAL <span className={styles.titleHighlight}>SKILLS</span></h2>
      </div>

      <div className={styles.gridContainer}>
        {technicalSkillsRows.map((row, rowIndex) => (
          <div key={`row-${rowIndex}`} className={styles.row}>
            {row.map((skill) => {
              const isActive = activeItem === skill.id;
              return (
                <div 
                  key={skill.id} 
                  className={`${styles.card} ${isActive ? styles.cardActive : ""} 
                    ${skill.id === "github" || skill.id === "nextjs" ? styles.githubCard : ""}`}
                  onMouseEnter={() => setActiveItem(skill.id)}
                >
                  {renderIcon(skill)}
